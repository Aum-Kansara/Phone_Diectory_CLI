...
mydb = connect(host = 'localhost',user = 'root',password = '' , database = 'phonedirectory' )
...
class Phoedirectory:
    ...
    def insert(self,name,num1,num2):
        try:
            mycursor.execute("select * from contactlist order by name")
            ...
            if not exists:
                if num2==None:
                    sql = "INSERT INTO contactlist(name,number1) VALUES (%s, %s)"
                    val = (name.lower(),num1)
                else:
                    sql = "INSERT INTO contactlist VALUES (%s, %s , %s)"
    ...
    def search(self):
        ...
        while not searchdone:
            ...
            cprint(st,"green")
            print()
            mycursor.execute("select * from contactlist where name LIKE '%"+st+"%' or number1 LIKE '%"+st+"%' or number2 LIKE '%"+st+"%'" )
    ...
    def delete(self,dl):
        ...
        for i in con:
            if dl in i[0] or i[1] or i[2]:
                mycursor.execute("DELETE FROM contactlist WHERE name='" + dl + "' or number1='" + dl + "' OR number2 ='" + dl + "'")
                mydb.commit()
...
while qt:
    ...
    if op==1:
        ...
        n2=None
        if input(f"Do you want to add another number of {name1}? Y/n :").lower()=="y":
